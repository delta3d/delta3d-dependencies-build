...
int main(int argc, char* argv[])
{
	...
	if(Type==ANIMATION)
	{
		...
		/*float duration = Ani->getDuration();
		printf("duration %f\n", duration);
		
		std::list<CalCoreTrack *>& tracks = Ani->getListCoreTrack();
		for (std::list<CalCoreTrack*>::const_iterator it = tracks.begin(); it != tracks.end(); ++it)
		{
			printf("\n Beginning track \n\n");
                        CalCoreTrack *track = *it;
                        std::vector<CalCoreKeyframe*> framesToReAdd;
                        while (track->getCoreKeyframeCount() > 0)
                        {
                        	CalCoreKeyframe * cckf = track->getCoreKeyframe(track->getCoreKeyframeCount()-1);
                        	track->removeCoreKeyFrame(track->getCoreKeyframeCount()-1);
                        	float time = cckf->getTime();
                        	float newTime = time - 1.03343f;
                        	if (newTime < 0.0f)
				  newTime += duration;
				printf ("time old %f new %f\n", time, newTime);
                        	cckf->setTime(newTime);
                        	framesToReAdd.push_back(cckf);
                        	//track->addCoreKeyframe(cckf);                        	
                        }
                        for (int i = 0; i < framesToReAdd.size(); ++i)
                        {
                        	track->addCoreKeyframe(framesToReAdd[i]);
                        }
		}*/
